...
Server::Server() : server_socket(-1), port(8080) {
...
void Server::set_port(int server_port) {
        port = server_port;
    }
...
int Server::bind_and_listen(int backlog ) {
    // Привязываем сокет
...
std::string MessageHandler::get_color_for_name(const std::string &name) {
    int hash = 0;
    for (char c : name) {
        hash = hash * 31 + c;
    }
...
void MessageHandler::broadcast_message(const std::string &message, int sender_socket) {
...
void MessageHandler::send_to_client(const std::string &message, int client_socket) {
    send(client_socket, message.c_str(), message.length(), 0);
}


std::string MessageHandler::receive_client_name(int client_socket) {
    char buffer[1024];
...
void MessageHandler::add_client(int client_socket, const std::string &client_name) {
    std::lock_guard<std::mutex> lock(clients_mutex);
    clients.push_back({client_socket, client_name});
}


void MessageHandler::remove_client(int client_socket) {
    std::lock_guard<std::mutex> lock(clients_mutex);
    clients.erase(
...
void MessageHandler::notify_join(int client_socket, const std::string &client_name) {
...
void MessageHandler::notify_leave(int client_socket, const std::string &client_name) {
...
void MessageHandler::process_messages(int client_socket, const std::string &client_name) {
    char buffer[1024];
    int bytes_received;
...
void MessageHandler::handle_client(int client_socket) {
