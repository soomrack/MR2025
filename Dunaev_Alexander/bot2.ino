...
#define base_speed 90 // базовая скорость 
float gain_p = 8.0;   // коэф. Р
float gain_d = 5.0;   // коэф. D
int search_speed = 110; // скорость при поиске линии
int light_threshold = 50; //Значение, при котором линия считается потерянной
int spiral_increase = 3; // инкремент шага спирали
int spiral_interval = 50; // интервал повышения шага
int search_timeout = 5000; // время на поиск линии
