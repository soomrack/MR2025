...
public:
  ...
  void updateOutput()
  {
    digitalWrite(pin, activation ? HIGH : LOW);
  }
};
...
public:
  const int pin = A0;
  int value = 0; // От 500 — темно, 260 и меньше — светло.
  void read() {
...
void regulateTemperature(Thermometer &therm, Fan &fan, Heater &heater)
{
  ...
  {
    fan.byHeat = true;
    heater.activation = false;
  } 
  else if (therm.temperature < limits.tempMin) {
    fan.byHeat = false;
    fan.byHeater = true;
    heater.activation = true;
  }
  else
  {
    heater.activation = false;
