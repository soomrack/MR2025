...
constexpr const char* ARDUINO_PORT = "/dev/ttyACM0";  // поменяйте на свой порт
constexpr speed_t     ARDUINO_BAUD = B9600;
constexpr const char* ARDUINO_LOG  = "arduino.log";
...
std::string processLogCommand(const std::string& cmd) {
    ...
    else if (cmd == "ARDUINO ALL") {
        std::ifstream f(ARDUINO_LOG);
        std::string line;
        while (std::getline(f, line))
            result.push_back(line);
    }
    else if (cmd.rfind("ARDUINO LAST ", 0) == 0) {
        try {
            int    minutes = std::stoi(cmd.substr(13));   // длина "ARDUINO LAST " == 13
            time_t now     = time(nullptr);
            ...
            return "[ERROR] usage: ARDUINO LAST <minutes>\n";
        }
    }
    else {
        return "[ERROR] commands: LOG ALL | LOG WARNINGS | LOG LAST <min> | ARDUINO ALL | ARDUINO LAST <min>\n";
    }
...
void arduinoLoop() {
    std::ofstream log(ARDUINO_LOG, std::ios::app);

    int fd = open(ARDUINO_PORT, O_RDWR | O_NOCTTY);  // O_RDWR, не O_RDONLY
    if (fd < 0) { logEvent("[ARDUINO] Cannot open " + std::string(ARDUINO_PORT)); return; }

    termios tty{};
    tcgetattr(fd, &tty);
    cfmakeraw(&tty);                       // сырой режим одной функцией
    cfsetispeed(&tty, ARDUINO_BAUD);
    cfsetospeed(&tty, ARDUINO_BAUD);       // обязательно обе стороны
    tty.c_cc[VMIN] = 1; tty.c_cc[VTIME] = 0;
    tcsetattr(fd, TCSANOW, &tty);

    logEvent("[ARDUINO] Opened " + std::string(ARDUINO_PORT));
...
int main() {
    ...
    std::thread ard(arduinoLoop);
    acceptLoop();

    serverRunning = false;
    if (mon.joinable()) mon.join();
    if (ard.joinable()) ard.join();
