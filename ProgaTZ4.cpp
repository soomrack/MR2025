...
{ 
    ...
    Alice.salary = 170000;              // Зарплата 150к
...
void Bob_init() 
{
    Bob.account = 800000;              // На счете у Боба
    Bob.salary = 170000;               // Зарплата 170к
    Bob.bank_percent = 0.06;           // Процент в банке 6 %
    Bob.wastes = 40000;                // Базовые расходы 40к
...
{
    // Базовые расходы(еда, средства первой необходимости и одежда)
...
void print_results() 
{
    ...
    if ((Alice.account + alice_equity) > (Bob.account)) 
    {
...
void simulation() 
{
    ...
    {
        //Накопления и расходы Алисы
        Alice_salary(year, month);// Зарплата Алисы
        Alice_deposite(year, month);// Расчет процентов Алисы
        Alice_expenses(year, month);// Расчет расходов Алисы на еду, средства первой необходимости и одежду
        Alice_flat_expenses(year, month);// Расходы на события в 
        Alice_life_events(year, month);// жизни Алисы

        //Накопления и расходы Боба
        Bob_salary(year, month);// Зарплата Боба
        Bob_deposite(year, month);// Расчет процентов Боба
        Bob_expenses(year, month);// Расчет расходов Боба на еду, средства первой необходимости и одежду
        Bob_life_events(year, month);// Расходы на события в жизни Боба
