# ДОКУМЕНТАЦИЯ — CHAT MAX 228.0
Чат с дистанционным управлением роботом через UART

Платформа: AARCH64 (Raspberry Pi 5)

Клиенты (протестировано на): Windows 11, Ubuntu 25.10, NixOS 26.05, Arch Linux for ARM (server)
...
Chat MAX 228.0 — многопользовательский чат-сервер на C, работающий на Raspberry Pi 5 / Raspbian.
...
```
  КЛИЕНТ                                СЕРВЕР (Raspberry Pi 5)           ARDUINO MEGA 2560
  ─────────────────────────────     ─────────────────────────────     ─────────────────────────────
                 ...
                 └────────────────────────► ┌─────────────────────────────┐
                                    ...
                                    /dev/ttyAMA0   │
                                                   ...
                                                   └──────────────┬──────────────┘
                                                                  │
                                                   ┌──────────────▼──────────────┐
                                                   │  Serial1.read() побайтово   │
                                                   │  накапливает в cmd_buf[]    │
                                                   │  до символа '\n'            │
                                                   └──────────────┬──────────────┘
                                                                  │
                                                   ┌──────────────▼──────────────┐
...
### Обратный поток: ток → лог

```
  ARDUINO MEGA 2560                 СЕРВЕР (Raspberry Pi 5)
  ─────────────────────────────     ─────────────────────────────
  ...
  └──────────────┬──────────────┘
                 ...
                 └────────────────────────> ┌─────────────────────────────┐
                                            ...
                                            └──────────┬────────┬─────────┘
                                                       │        │
                                            left≤3.0А  │        │  left > 3.0 А
                                                       │        │  или right > 3.0 А
                                   ┌───────────────────▼──┐  ┌──▼────────────────────────┐
                                   │  motor_log("INFO",   │  │  motor_log("WARNING",     │
                                   │  "Current:           │  │  "LEFT motor overcurrent: │
                                   │   left=1.23A         │  │   3.45A")                 │
                                   │   right=0.98A")      │  │                           │
                                   └──────────────────────┘  └───────────────────────────┘
                                              │                          │
...
[2026-03-04 12:00:05] User: ENTERED DRIVE MODE
[2026-03-04 12:00:06] User: KEY 'w' -> FORWARD speed=70%
[2026-03-04 12:00:07] INFO: Current: left=1.23A right=1.18A
[2026-03-04 12:00:08] User: KEY ' ' -> STOP speed=70%
[2026-03-04 12:00:09] WARNING: LEFT motor overcurrent: 3.45A
[2026-03-04 12:00:10] User: EXIT DRIVE MODE — motors stopped
```
...
./chat_server 127.0.0.1   # подключается к 127.0.0.1:5000
```
...
./chat_client 192.168.0.1   # подключается к 192.168.0.1:5000
```
