...
void alice_init() {
	...
	alice.apartment.HasApartment = true;
...
void bob_init() {
	...
	bob.apartment.cost = 10 * 1000 * 1000;
	bob.apartment.HasApartment = false;
...
void alice_mortgage(const int& year, const int& month) {
	if (alice.bank.mortgage_amount > 0) {
		alice.bank.mortgage_amount -= alice.bank.mortgage_payment;
		alice.bank.account -= alice.bank.mortgage_payment;
	}
	else if (alice.bank.mortgage_amount != 0) {
		std::cout << "Алиса закрыла ипотеку: " << month << " месяц " << year << " год\n";
		alice.bank.mortgage_amount = 0;
	}
}
...
void alice_investment(const int& year, const int& month) {
	...
	alice.bank.investment += alice.bank.account - 10 * 1000;
	alice.bank.account == 10 * 1000;
}
...
void bob_apartment(const int& month) {
	if (month == 10) bob.apartment.cost *= 1.07;
}
...
void bob_utilities(const int& month) {
	...
	if (!bob.apartment.HasApartment) bob.bank.account -= bob.apartment.rent;    // Пока нет квартиры - арендует
...
void bob_investment(const int& year, const int& month) {
	...
	if (!bob.apartment.HasApartment && bob.bank.investment > bob.apartment.cost) {       // Если хватает инвестиций снимает и покупает
		bob.bank.investment -= bob.apartment.cost;
		bob.apartment.HasApartment = true;
...
void alice_print() {
	...
	if (alice.apartment.HasApartment) std::cout << "Есть квартира\n";
...
void bob_print() {
	...
	if (bob.apartment.HasApartment) std::cout << "Есть квартира\n";
	else std::cout << "Нет квартиры\n";
}
