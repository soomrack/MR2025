    ...
    // Создаём копию порта, чтобы избежать проблемы с указателем
        int* portCopy = new int(listen_port);
    
    	HANDLE hServer = CreateThread(NULL, 0, ServerThread, portCopy, 0, NULL);
    	if (!hServer) {
