...
void findLine() {
    ...
    while (isSearching && millis() - startTime < 10000) {
        ...
        int l_val = searchSpeed - spiralStep;
        int r_val = searchSpeed - spiralStep;

        if (spiralDirect) {
            setMotors(l_val, searchSpeed);    // Вправо
        } else {
            setMotors(searchSpeed, r_val);   // Влево
        }
...
bool lineLost() {
    ...
    if (r_read > r_threshold || l_read > l_threshold) {
        lineFound = true;
        lastDirect = (l_read > l_threshold) ? 0 : 1;
    } else {
        lineFound = false;
    }
