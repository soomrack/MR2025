...
struct {
    RUB sum;
    const RUB minimum_balance;
    const double year_percent;
} deposit_features = { 0, 40000, 9.0 };


const double inflation_month_coefficient = 1.0 + 0.6 / 100;
const double indexation_quarter_coefficient = 1.0 + 1.9 / 100;
...
void Person::food() {
    ...
    food_spendings *= inflation_month_coefficient;
}


void Person::car_sharing() {
    bank_account -= car_sharing_month_spending;
    car_sharing_month_spending *= inflation_month_coefficient;
}
...
void Mortage_person::income(const int year, const int month) {
    ...
    if (month == 1 or month == 4 or month == 7 or month == 10) {
        income_amount *= indexation_quarter_coefficient; //indexation
    }
...
void Rent_person::deposit() {
    ...
    if (bank_account > deposit_features.minimum_balance) {
        RUB month_deposit = bank_account - deposit_features.minimum_balance;
