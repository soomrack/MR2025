...
// 
// убрать функцию инфляции и делать инфляцию в каждой функции трат
// добавить комментарии к expenses (// еда, одежда и тд)
// комментарии оформить красиво
// убрать отладочные printf
// === TODO ===
...
void alice_init() {
	...
	alice.expenses = 50 * 1000 * 100;               // Траты в месяц у Алисы (еда, одежда и тд)
...
void alice_income(const int year, const int month) {
	...
	if (month == 12) {
		alice.money += alice.work.new_year_award;		// Новогодняя премия
...
void alice_expenses(const int month) {
	...
	if (month == 12) {
		alice.expenses *= (1.0 + inflation);	// Инфляция раз в год на ежемесячные траты
	}
...
void alice_mortage(int year, int month) {
	...
	alice.home.mortage_total += alice.home.mortgage;	//Общая сумма заплаченая Алисой за ипотеку
}


void alice_money_transfer(int year, int month) {
	alice.bank_account += alice.money;	// Перевод оставшихся денег на счет в банке
	alice.money = 0;
}
...
void bob_init() {
	...
	bob.expenses = 50 * 1000 * 100;               // Траты в месяц у Боба (еда, одежда и тд)
...
void bob_income(const int year, const int month) {
	...
	if (month == 12) {
		bob.money += bob.work.new_year_award;		  // Новогодняя премия
...
void bob_expenses(const int month) {
	...
	if (month == 12) {
		bob.expenses *= (1.0 + inflation);		// Инфляция раз в год на ежемесячные траты
	}
...
void bob_rent(const int month) {
	...
	if (month == 12) {
		bob.home.rent *= (1.0 + inflation);		// Инфляция раз в год на раенду жилья
	}
}


void bob_money_transfer(int year, int month) {
	bob.bank_account += bob.money;		// Перевод оставшихся денег на счет в банке
	bob.money = 0;
}
...
void bob_can_buy_apartment(int year, int month) {
	...
	if (month == 12) {
		bob.home.apartment_price *= (1.0 + inflation);		// Инфляция раз в год на стоимость квартиры
	}
}
...
void simulation() {
	...
	while (!(year == end_year && month == end_month)) {
		...
		alice_expenses(month);		// Еда, одежда
		alice_mortage(year, month);
		alice_money_transfer(year, month);

		bob_income(year, month);
		bob_expenses(month);		// Еда, одежда
		bob_rent(month);
