...
// Ниже идут функции, связанные с Алисой

void alice_income(const int year, const int month)
{
    if(month == 9) {
        alice.income = alice.income * 1.07;  // Индексация зарплаты
    }
...
void alice_deposit(const int month)
{
   if (alice.bank_account > alice.initial_capital) {
        RUB remaining_balance = alice.bank_account - alice.initial_capital;
        alice.bank_account = alice.initial_capital;  
        alice.deposit += remaining_balance;  
    }
...
//Ниже идут функции, связанные с Бобом

void bob_income(const int year, const int month)
{
    if(month == 9){
        bob.income = bob.income * 1.07;  // Индексация зарплаты
    }
...
void bob_deposit(const int month)
{
   if (bob.bank_account > bob.initial_capital) {
        RUB remaining_balance = bob.bank_account - bob.initial_capital;
        bob.bank_account = bob.initial_capital;  
        bob.deposit += remaining_balance;  
    }
