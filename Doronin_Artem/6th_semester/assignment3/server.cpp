...
std::string lastSensorValue = "No sensor data";
    ...
    try {
        while (true) {
            ...
            if (header.size > 10240) {
                throw std::runtime_error("Message too large (garbage received)");
            }
            ...
            switch (static_cast<MessageType>(header.type)) {
                ...
                case MessageType::Text:
                    std::cout << "Received (and logged): " << data << std::endl;
                    writeLog(data);
                    
                    {
                        std::lock_guard<std::mutex> lock(sensorMutex);
                        lastSensorValue = data;
                    }
                ...
                case MessageType::StatusRequest: {
                    ...
                    {
                        std::lock_guard<std::mutex> lock(sensorMutex);
                        currentSensor = lastSensorValue;
                    }

                    std::string msg = "Temp: " + std::to_string(t).substr(0, 4) + "C, RAM: " + std::to_string(ram) + "%, " + currentSensor;
        ...
        std::cerr << "Client disconnected (" << myName << "): " << e.what() << std::endl;
        close(clientSock);
        std::lock_guard<std::mutex> lock(clientsMutex);
            ...
            if (c.autoLog && c.autoLogInterval > 0) {
                ...
                if (c.autoLogCounter >= c.autoLogInterval) {
                    ...
                    if (!fetched) {
                        ...
                        ram = getRAMUsage();
                        fetched = true;
                        isWarning = (t > TEMP_WARNING_THRESHOLD) || (ram > RAM_WARNING_THRESHOLD);
                        
                        std::string sysMsg = "System -> Temp: " + std::to_string(t).substr(0, 4) + "C, RAM: " + std::to_string(ram) + "%";
                        writeLog(sysMsg, isWarning);
                    }
                    
                    std::string currentSensor;
                    {
                        std::lock_guard<std::mutex> sensLock(sensorMutex);
                        currentSensor = lastSensorValue;
                    }
