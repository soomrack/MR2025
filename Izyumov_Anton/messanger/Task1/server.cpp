...
void PollLoop::handleServerCommand(Client* client, const std::string& command) {
    ...
    if (command == "/server time") {
        time_t now = time(nullptr);
        std::string time_str = ctime(&now);
        time_str.pop_back(); // убираем перевод строки
        std::string response = "Server time: " + time_str;
        client->send(response);
        std::cout << "    Отправлен ответ о времени клиенту fd=" << client->getFd() << std::endl;
    } else {
        client->send("Неизвестная команда серверу");
    }
}
