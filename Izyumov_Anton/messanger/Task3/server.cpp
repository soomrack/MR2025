...
std::string SystemInfo::getCpuTemp() {
    ...
    std::ostringstream oss;
    oss << "Температура процессора: " << std::fixed << std::setprecision(2) << temp_celsius << " °C";
    return oss.str();
}
...
public:
    ...
    bool open(const char* portname, speed_t baud);
    int getFd() const;
    void processInput();
    std::string getLastLine() const;
    int getJoyX() const { return joy_x_; }
    int getJoyY() const { return joy_y_; }
};

SerialPort::SerialPort() : fd_(-1), buffer_(), last_line_(), joy_x_(0), joy_y_(0) {}

SerialPort::~SerialPort() {
    if (fd_ >= 0) close(fd_);
}

bool SerialPort::open(const char* portname, speed_t baud) {
    fd_ = ::open(portname, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (fd_ < 0) {
        std::cerr << "Не удалось открыть порт " << portname 
                  << ": " << strerror(errno) << std::endl;
        return false;
    }
    std::cout << "Порт " << portname << " открыт, fd=" << fd_ << std::endl;
...
void SerialPort::processInput() {
    char buf[256];
    int n = read(fd_, buf, sizeof(buf));
    if (n < 0) {
        if (errno != EAGAIN) perror("read serial");
        return;
    }
    if (n == 0) return;
...
std::string SerialPort::getLastLine() const {
    return last_line_;
}

void SerialPort::parseLine(const std::string& line) {
    size_t xpos = line.find("x:");
    size_t ypos = line.find("y:");
    if (xpos != std::string::npos && ypos != std::string::npos) {
        auto extract = [&](size_t pos) -> int {
            size_t start = pos + 2;
            while (start < line.size() && isspace(line[start])) ++start;
            size_t end = start;
            while (end < line.size() && (isdigit(line[end]) || line[end] == '-')) ++end;
            if (start < end) return std::stoi(line.substr(start, end - start));
            return 0;
        };
        joy_x_ = extract(xpos);
        joy_y_ = extract(ypos);
...
void PollLoop::handleInput(int fd) {
    ...
    else if (fd == serial_port_.getFd()) {
        serial_port_.processInput();
        return;
    }
    else {
...
void PollLoop::handleServerCommand(Client* client, const std::string& command) {
    ...
    else if (subcmd == "joystick") {
        std::ostringstream oss;
        oss << "Джойстик X: " << serial_port_.getJoyX() << ", Y: " << serial_port_.getJoyY();
        response = oss.str();
    }
    else {
        response = "Неизвестная команда серверу. Доступные: time, cpu, cpu_temp, ram, rom, joystick";
    }
...
bool PollLoop::initSerial(const char* portname) {
    if (!serial_port_.open(portname, B9600)) {
        std::cerr << "Не удалось открыть последовательный порт " << portname << std::endl;
        return false;
    }
    int fd = serial_port_.getFd();
    struct pollfd pfd;
    pfd.fd = fd;
    pfd.events = POLLIN;
    poll_fds_.push_back(pfd);
    std::cout << "[*] Последовательный порт " << portname << " открыт, fd=" << fd << std::endl;
    return true;
}
...
bool Application::initialize() {
    ...
    poll_loop_.initSerial("/dev/serial0");  // или /dev/ttyACM0
