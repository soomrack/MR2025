#define _WIN32_WINNT 0x0600 //Директива препроцессора(минимальная версия виндовс для которого предназначена программа, каждая имеет свой код, конкретно эта windows vista,2008)
#define _CRT_SECURE_NO_WARNINGS //Для предотвращения предупреждений об опасных функциях 
#define _WINSOCK_DEPRECATED_NO_WARNINGS //как и предыдущая, но для сетевых функций; Это все написано в define потому что он включается до компиляции, и во время компиляции никаких ошибок не вылезет

#include <iostream> //ввод/вывод в консоль
#include <string> //работа со строками
#include <thread> //многопоточность
#include <chrono> //работа со врменем и задержками
#include <conio.h> //управление консолью клавишами без энтер, например чтобы робот двигался
#include <fstream> //для работы с файлами
#include <sstream> //работа со строковыми потоками
#include <iomanip> //формирует имена файлов при запросе статистики
#include <ctime> // работа с датой и временем 
#include <vector> //динамические массивы
#include <memory> //безопасное управление памятью
#include <unordered_map> //хранение режимов работы и нахождение их пономеру
#include <functional>

#include <winsock2.h> //для работы с сетью
#include <windows.h> //связь программы с ОС
#include <ws2tcpip.h> //дополнительные винсок функции

#pragma comment(lib, "ws2_32.lib") //Подключает библиотеку ws2_32.lib к проекту на этапе компиляции, чтобы не вводить в консоли вручную

using namespace std; //Позволяет не писать std:: перед каждым элементом стандартной библиотеки. стд-контейнер, где лежат все библиотеке 

// ========== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ОБЪЯВЛЕНИЯ) ==========
string utf8_to_cp1251(const string& utf8_str); //Преобразует строку из UTF-8 в кодировку Windows-1251.
void set_console_encoding(); //Устанавливает кодировку консоли Windows на Windows-1251.
void clear_screen(); //Очищает экран консоли.
void print_header(); //Выводит шапку (заголовок) программы в консоль.
string get_current_filename(); //Генерирует уникальное имя файла на основе текущей даты и времени.
void save_to_file(const string& content); //Позволяет не терять результаты запросов к роботу — всё сохраняется в отдельные текстовые файлы с меткой времени.

// ========== ИНТЕРФЕЙС РЕЖИМА РАБОТЫ ==========
class IClientMode { //класс интерфейса 
public: //Открывает доступ ко всему, что идёт после неё.
    virtual ~IClientMode() = default; //для того, чтобы не было утечки памяти и удалялились наследники при удалении объекта
    virtual void execute(class RobotClient& client) = 0; // Я для опредленного действия есть метод, прописанный в другом классе, а через эту команду я просто говорю, что такой нужный мне метод, а он уже сам знает как работать
    virtual string get_name() const = 0; //прошлая строчка говорила что у каждого метода есть работа, а этот метод говорит их имя(должность)
};

// ========== КЛАСС RobotClient (ОБЪЯВЛЕНИЕ) ==========
class RobotClient { //класс для связи с роботом по сети(типа пульт управления)
private: //потому что должен работать только класс со следующими командами
    SOCKET sock_; // это внутренний "кабель" (дескриптор), по которому программа отправляет и получает данные от сервера
    string server_ip_; //это строка, которая хранит IP-адрес Raspberry Pi (робота), к которому нужно подключиться
    int port_; // номер порта для подлкючения к серверу
    bool connected_; //либо тру либо фолс, подключен к дрону или нет
    bool receive_mode_; // либо тру либо фолс(отправил и ждешь ответа или отправил и не ждешь ответа)
    static const int recv_timeout_ms_ = 5000; //время ожидания ответа от сервера

public:
    RobotClient(const string& ip, int p); // конструктор, который вызывается при создании клиента, чтобы сохранить адрес и порт робота и подготовить объект к работе (установить начальные значения флагов и сокета).
    ~RobotClient(); //тильда-знак отрицания в логике; деструктор, который автоматически закрывает сокет и чистит Winsock при уничтожении объекта, чтобы ресурсы не "утекали"

    bool connect_to_server(); //либо тру либо фолс подлючилось или нет
    void disconnect(); //отключение от сервера 
    string send_command(const string& cmd); //отправка команды управления(например движение по клавишам) и получение обратно
    string send_request(const string& cmd); //отправка запроса например статистика и получение ответа
    string send_monitoring_request(const string& cmd); //отправка запроса на мониторинг и получение обратно
    bool is_connected() const; //проверка подключения(тру или фолс)
};

// ========== РЕЖИМЫ РАБОТЫ (ОБЪЯВЛЕНИЯ) ==========
class ControlMode : public IClientMode { //новый класс наследует публичный интерфейс у другого класса
public: //доступный всем функции
