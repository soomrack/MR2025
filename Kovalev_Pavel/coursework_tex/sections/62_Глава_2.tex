...
Приложение имеет три основных состояния, определяемых перечислением \texttt{GameState} \cite{gregory2026}:
\begin{itemize}
...
\begin{enumerate}
	...
	\item Если есть попадания и неуязвимость прошла (\texttt{invincible\_until < level\_time}), вызывается \texttt{player.hit(1)} и устанавливается \texttt{invincible\_until = level\_time + 0.5f}. Это обычная механика для подобных игр \cite{terraria_wiki_inv_frame}\cite{calamity_wiki_inv_frame}.
